...
1. **简短明确**：用最简洁的文字描述字段含义
2. **与需求一致**：必须与需求文档中的字段名称/含义保持一致
3. **避免冗余**：不重复表名、不加"字段"后缀
4. **枚举值标注**：状态字段需标注所有枚举值的含义
...
status = db.Column(db.Integer, comment='状态 0禁用 1启用')
is_enabled = db.Column(db.Integer, comment='是否启用')

# ❌ 错误（冗余）
username = db.Column(db.String(64), comment='用户名username')
status = db.Column(db.Integer, comment='状态')

# ❌ 错误（与需求不一致）
user_name = db.Column(db.String(64), comment='用户姓名')  # 需求文档写的是"用户名"
```
...
| 名称 | 中文含义 | `用户名`、`角色名称` |
| 代码 | 中文含义 | `角色代码`、`权限代码` |
| 状态 | 中文含义+枚举 | `状态 0禁用 1启用` |
| 外键 | 中文含义+ID | `角色ID`、`权限ID` |
| 时间 | 中文含义 | `创建时间`、`更新时间`、`最后登录时间` |
| 计数 | 中文含义 | `登录次数`、`访问次数` |
| 标志 | 中文含义 | `是否启用`、`是否删除` |
...
| 文件 | 说明 |
|:-----|:-----|
| `docs/API文档/swagger_template.md` | 接口文档模板 |
...
使用 `gunicorn_loader.py` 作为启动器，从配置文件读取地址和端口：
...
class LastMileCargoType(db.Model):
    __tablename__ = 'cbjsq_last_mile_cargo_type'
```
...
if __name__ == '__main__':
    from common.confs import app_conf
